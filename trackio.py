...
def detect_tracks_intelligent(y, sr,
                              ...
                              y_norm=None):
    """
    Unified Single Source of Truth for identifying tracks.
    Returns tracking intervals with user-configurable boundary padding.
    """
    if y_norm is None:
    ...
    # Efficient Vectorized contiguous segment identification
    ...
    # Merge closely spaced segments based on run-time parameter
    ...
    # Filter by duration and safely apply runtime padding boundary constraints
    final_tracks = []
...
def analyze_audio(y, sr, min_track_dur_ms=2000, merge_gap_ms=200, padding_ms=1000, silence_thresh=None):
    """Perform clear visualization reporting matching target metrics output exactly."""
    print("\n" + "="*50)
...
def process_audio(y, sr, file_path, output_dir, prefix, quality="medium", format="wav",
                  min_silence_len=1500, merge_gap_ms=200, min_track_dur_ms=2000, padding_ms=1000, silence_thresh=None):
...
def main():
    ...
    parser.add_argument("--min-silence-len", type=int, default=1500, help="Min silence in ms to separate major tracks, def 1500 ms")
    parser.add_argument("--merge-gap", type=int, default=200, help="Max gap in ms between segments to merge them, def 200 ms")
    parser.add_argument("--min-track-dur", type=int, default=2000, help="Min duration in ms for a valid track, def 2000 ms")
    parser.add_argument("--padding", type=int, default=1000, help="Buffer padding in ms added to both ends of a track, def 1000 ms")
    parser.add_argument("--silence-thresh", type=float, default=None, help="Override automatic threshold. Lower is more sensitive.")
    ...
    if args.analyze:
        analyze_audio(y, sr, min_track_dur_ms=args.min_track_dur, merge_gap_ms=args.merge_gap, padding_ms=args.padding, silence_thresh=args.silence_thresh)
    else:
        process_audio(
            ...
            merge_gap_ms=args.merge_gap, min_track_dur_ms=args.min_track_dur, padding_ms=args.padding, silence_thresh=args.silence_thresh
        )
